...
object MockData {
    ...
    fun getNews(id: Int?): NewsData = topNewsList.first { it.id == id }
    ...
    fun stringToDate(publishedAt: String): Date {
            val date = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.ENGLISH).parse(publishedAt)
